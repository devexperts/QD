...
public abstract class RMIService<T> implements RMIObservableServiceDescriptors, RMIChannelSupport<T> {
    ...
    public synchronized void removeServiceDescriptorsListener(RMIServiceDescriptorsListener listener) {
        ...
        if (descriptors.isEmpty())
            return;
        List<RMIServiceDescriptor> unavailableDescriptors = Collections.unmodifiableList(
            descriptors.stream()
                .map(RMIServiceDescriptor::toUnavailableDescriptor)
                .collect(Collectors.toList())
        );
        listener.descriptorsUpdated(unavailableDescriptors);
    }
