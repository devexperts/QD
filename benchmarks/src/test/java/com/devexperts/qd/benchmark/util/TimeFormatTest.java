...
public class TimeFormatTest extends TestCase {
    ...
    public void testZoneStability() {
        ...
        TestCase.assertEquals(1197450000000L, GMT.parse("2007-12-12T12+0300").getTime());
        // make sure it does not break the default formatting of GMT timezone
        TestCase.assertEquals("20140618-130218+0000", GMT.withTimeZone().format(new Date(1403096538000L)));
    }

    public void testTimePeriods() throws InvalidFormatException {
        TestCase.assertEquals(0L, TimePeriod.valueOf("0").getTime());
        TestCase.assertEquals(1000L, TimePeriod.valueOf("1s").getTime());
        TestCase.assertEquals(123L, TimePeriod.valueOf(".123456789").getTime());
        TestCase.assertEquals("PT1.235S", TimePeriod.valueOf("1.23456789").toString());
        TestCase.assertEquals(TimePeriod.valueOf("1.23456789"), TimePeriod.valueOf("0d0h0m1.235s"));
    }
    ...
    public void testBadTimePeriods() {
        ...
        for (String s : badValues) {
            try {
                ...
                TestCase.fail("Parsed bad value: " + s);
    ...
    public void testGoodTimePeriods() {
        ...
        for (String s : goodValues) {
            try {
                ...
                TestCase.fail("Couldn't parse: " + s);
            }
        }
    }
    ...
    public void testEqualDateTimes3() {
        ...
        TestCase.assertEquals(12, today.get(Calendar.HOUR_OF_DAY));
        TestCase.assertEquals(34, today.get(Calendar.MINUTE));
        TestCase.assertEquals(56, today.get(Calendar.SECOND));
    ...
    public void testNoTimeZoneFormat() {
        ...
        Date d2 = GMT.parse(s);
        TestCase.assertEquals(GMT.withTimeZone().format(d1), GMT.withTimeZone().format(d2));
    }

    public void testDateOutsideIsoRangeFormat() {
        Date d1 = new Date(Long.MIN_VALUE);
        TestCase.assertEquals(Long.toString(Long.MIN_VALUE), TimeFormat.GMT.format(d1));

        Date d2 = new Date(Long.MAX_VALUE);
        TestCase.assertEquals(Long.toString(Long.MAX_VALUE), TimeFormat.GMT.format(d2));

        Date d3 = new Date(TimeUtil.DAY << 32);
        TestCase.assertEquals(Long.toString(d3.getTime()), TimeFormat.GMT.format(d3));
    }
    ...
    public void testIsoFormat() {
        ...
        // test format
        TestCase.assertEquals("2002-05-30T09:30:10.000-06:00",
    ...
    public void testTimeWithoutTimeZone() {
        ...
        TestCase.assertEquals(MSD.parse("20120406-182355+0400"), withoutTZ);
    }

    public void testTimeWithMillis() {
        TimeFormat MSD = TimeFormat.getInstance(TimeZone.getTimeZone("Europe/Moscow"));
        TestCase.assertEquals(1333737666231L, MSD.parse("20120406-224106.231").getTime());
        TestCase.assertEquals(1333737666231L, MSD.parse("20120406-224106.231+0400").getTime());
    }

    public void testZero() {
        TestCase.assertEquals(0, GMT.parse("0").getTime());
        TestCase.assertEquals("0", GMT.withTimeZone().format(new Date(0)));
        TestCase.assertEquals("0", GMT.withMillis().format(new Date(0)));
        TestCase.assertEquals("0", GMT.withTimeZone().withMillis().format(new Date(0)));
        TestCase.assertEquals("0", GMT.format(new Date(0)));
    }
    ...
    public void testEquivalence() throws ParseException {
        for (String tzName : "GMT,GMT+01:30,GMT-01:30,Europe/Moscow,America/Chicago".split(",")) {
            ...
            TestCase.assertSame(format.withMillis().withTimeZone(), format.withTimeZone().withMillis());
            TestCase.assertSame(format.asFullIso(), format.withMillis().asFullIso());
            TestCase.assertSame(format.asFullIso(), format.withTimeZone().asFullIso());
            TestCase.assertSame(format.asFullIso(), format.withMillis().withTimeZone().asFullIso());
    ...
    public void testDSTEquivalence() throws ParseException {
        ...
        TestCase.assertTrue(chicagoTz.getOffset(begin) != chicagoTz.getOffset(end));
    ...
    public void testWeirdOffset() throws ParseException {
        ...
        TestCase.assertTrue(moscowTz.getOffset(begin) % 60_000 != 0);
        long end = sdf.parse("19190701-040000").getTime();
        TestCase.assertTrue(moscowTz.getOffset(end) % 60_000 == 0);
        ...
        long expected) throws ParseException
    {
        ...
        TestCase.assertEquals(canonical, format.format(expected));
        TestCase.assertEquals(canonical, format.format(new Date(expected)));
        TestCase.assertEquals(reversed, format.parse(canonical).getTime());
