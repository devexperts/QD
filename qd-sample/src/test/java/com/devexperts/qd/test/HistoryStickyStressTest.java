...
/**
 * Concurrent stress test for the QDHistory collector with sticky subscription enabled.
 ...
 * around sticky-add, chain repair and matrix rehash. Subscriptions use a mix of
 * discrete time-depth levels so the History-specific time-aware sticky paths are
 * also stressed.
 *
 * Used for manual start-up only, 3 min long.
 */
@Ignore
public class HistoryStickyStressTest {
    ...
    private static final long STICKY_PERIOD_MS = 2_000;  // 1 s
    ...
    private static final long BG_ROTATION_INTERVAL_MS = STICKY_PERIOD_MS / 2; // 1 s
    private static final long QUIET_WINDOW_MS = STICKY_PERIOD_MS;             // 2 s
    private static final long BURST_WINDOW_MS = 1_000;
    private static final long GLOBAL_QUIET_INTERVAL_MS = STICKY_PERIOD_MS * 4;  // every 8 s
    private static final long GLOBAL_QUIET_DURATION_MS = STICKY_PERIOD_MS * 2;  // pause 4 s

    // Subscription-time levels.
    private static final long[] DEPTH_LEVELS = {
        0L, 100L, 1_000L, 10_000L, 100_000L, 1_000_000L, Long.MAX_VALUE
    };
    ...
    private void runStress(QDFactory factory) throws InterruptedException {
        ...
        DataScheme scheme = new TestDataScheme(1, rnd.nextInt(), TestDataScheme.Type.HAS_TIME);
        ...
        // symbols at a random depth. Periodically replaces one agent with a fresh one. This:
        ...
        //  - Mixed depths across bg agents → total time is pinned by the deepest one,
        //    so removing any of them forces minSubTimeFromAgents recomputation.
        ...
        for (int i = 0; i < ROTATING_BG_COUNT; i++) {
            bgSlots[i] = newBackgroundAgent(history, record, ciphers, symbols, symbolsActive.get(), rnd);
        }
        Thread bgRotator = new Thread(() -> {
            try {
                for (int i = 0; getCurrentTimeInMillis() < deadline && failure.get() == null; i = (i + 1) % bgSlots.length) {
                    ...
                    QDAgent newAgent = newBackgroundAgent(history, record, ciphers, symbols, symbolsActive.get(), rnd);
            ...
            Thread worker = new Thread(() -> {
                ...
                try {
                    while (getCurrentTimeInMillis() < deadline && failure.get() == null) {
                        awaitNotQuiet(globalQuiet, deadline, failure);
                        burst(random, history, record, ciphers, symbols, symbolsActive,
            ...
            try {
                ...
                while ((currentTime = getCurrentTimeInMillis()) < deadline && failure.get() == null) {
                    ...
                    for (int i = 0; i < count; i++) {
                        ...
                        // History records carry a time; sample from same depth pool used for subs
                        // so events occasionally fall within agents' subscription ranges.
                        c.setTime(randomDepth(random));
    ...
    private static QDAgent newBackgroundAgent(
        QDHistory history, DataRecord record, int[] ciphers, String[] symbols, int active, Random rnd)
    {
        QDAgent agent = history.agentBuilder().build();
        RecordBuffer sub = new RecordBuffer(RecordMode.HISTORY_SUBSCRIPTION);
        long depth = randomDepth(rnd);
        for (int s = 0; s < active; s++) {
            if (rnd.nextBoolean()) {
                sub.add(record, ciphers[s], symbols[s]).setTime(depth);
            }
        }
    ...
    private static void burst(Random rnd, QDHistory history, DataRecord record, int[] ciphers, String[] symbols,
        ...
        AtomicReference<Throwable> failure)
    {
        ...
        while (getCurrentTimeInMillis() < burstEnd && failure.get() == null) {
            ...
            for (int i = 0; i < count; i++) {
                picked[i] = rnd.nextInt(active);
            }

            agent.setSubscription(buildSub(record, ciphers, symbols, picked, randomDepth(rnd)));

            // Re-subscribe the SAME agent to (a subset of) the same (k, rid) entries with a different depth.
            if (rnd.nextBoolean()) {
                int subCount = 1 + rnd.nextInt(picked.length);
                int[] sub = new int[subCount];
                for (int i = 0; i < subCount; i++) {
                    sub[i] = picked[rnd.nextInt(picked.length)];
                }
                agent.addSubscription(buildSub(record, ciphers, symbols, sub, randomDepth(rnd)));
            }
    ...
    private static RecordBuffer buildSub(DataRecord record, int[] ciphers, String[] symbols, int[] picked, long depth) {
        RecordBuffer sub = new RecordBuffer(RecordMode.HISTORY_SUBSCRIPTION);
        for (int idx : picked) {
            sub.add(record, ciphers[idx], symbols[idx]).setTime(depth);
        }
        return sub;
    }

    private static long randomDepth(Random rnd) {
        return DEPTH_LEVELS[rnd.nextInt(DEPTH_LEVELS.length)];
    }
