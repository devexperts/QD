* JMX tools [optional dependency]

JMX tools is licensed under "Oracle Binary Code License Agreement for Java SE and JavaFX Technologies"
(see https://www.oracle.com/downloads/licenses/java-se-archive-license.html) and can be downloaded manually as
a part of the "Java Management Extensions 1.2 Reference Implementation" software from
https://www.oracle.com/java/technologies/java-archive-downloads-java-plat-downloads.html

* Dual-license dependencies

- Jetty
This work is dual-licensed under Apache Software License - Version 2.0 and Eclipse Public License - Version 1.0. 
You can choose between one of them if you use this work. The license that we chose is in licenses folder.

- Java Servlet API
This work is dual-licensed under CDDL-1.1 and GPLv2 with classpath exception. 
You can choose between one of them if you use this work. The license that we chose is in licenses folder.

- Javax Annotation API
This work is dual-licensed under CDDL-1.1 and GPLv2 with classpath exception. 
You can choose between one of them if you use this work. The license that we chose is in licenses folder.

- JAXB 
This work is dual-licensed under CDDL-1.1 and GPLv2 with classpath exception. 
You can choose between one of them if you use this work. The license that we chose is in licenses folder.

- iStack Common Utility
This work is dual-licensed under CDDL-1.1 and GPLv2 with classpath exception. 
You can choose between one of them if you use this work. The license that we chose is in licenses folder.

- Java Native Access (JNA)
This work is dual-licensed under the LGPL, version 2.1 or later, or (from version 4.0 onward) the Apache License, version 2.0.
You can choose between one of them if you use this work. The license that we chose is in licenses folder.
